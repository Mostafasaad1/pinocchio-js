...
// EM_JS bodies are compiled as real JavaScript, unlike EM_ASM which
// has its tokens parsed as C++ first. Use these to throw typed JS
// exceptions from C++ code.
EM_JS(void, js_throw_range_error, (const char* msg), {
    throw new RangeError(UTF8ToString(msg));
});

EM_JS(void, js_throw_error, (const char* msg), {
...
PINOCCHIO_ALIGNED_STD_VECTOR(pinocchio::Force) jsToForceVector(const Model& model, const val& fext_js) {
    ...
    for (unsigned k = 0; k < n; ++k) {
        ...
        if (*endptr != '\0' || parsed < 0 || parsed >= model.njoints) {
            ...
            js_throw_range_error(msg.c_str());
            return fext;
        }
